...
The previous API relied on environment variables to manage `.egg` files for the CARLA Python API. The updated API and Docker configuration
now handle the Python API installation using `.whl` files, making these environment variables obsolete.

To build the API object, use one of the following static methods:
...
# Build from an existing CARLA client object
api = CarlaCDASimAPI.build_from_client(carla_client)

# or...

# Build from an existing CARLA world object
