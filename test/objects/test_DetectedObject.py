...
class TestDetectedObject(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # CARLA 0.10.0: Vehicles is now Car
        self.carla_actor.semantic_tags = [int(carla.CityObjectLabel.Car)]
    ...
    def test_builder(self):
        # Mock the type_id and attributes for CARLA 0.10.0
        self.carla_actor.type_id = "vehicle.ford.mustang"
        self.carla_actor.attributes = {'base_type': 'car'}
        ...
        assert detected_object.type == "CAR"
        ...
        # Replicate the rotation - CARLA Rotation(pitch=30, yaw=90, roll=45) -> [roll, pitch, yaw] in radians
        rotation_angles_deg = np.array([45.0, 30.0, 90.0])
        rotation_angles = np.deg2rad(rotation_angles_deg)
        assert np.allclose(detected_object.rotation, rotation_angles)
